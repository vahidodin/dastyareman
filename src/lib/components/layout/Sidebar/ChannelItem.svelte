...
>
	...
	>
		<div class="flex min-w-0 flex-1 items-center gap-1">
			<div>
				{#if channel?.type === 'dm'}
					{#if channel?.users}
						...
						<div class="flex ms-[0.0625rem] me-0.5 relative">
					...
					{:else}
						<UserIcon className="size-4 ms-1 me-0.5" strokeWidth="1.5" />
					{/if}
				{:else}
					<div class=" size-4 justify-center flex items-center ms-1">
			...
			<div class="text-start self-center min-w-0 flex-1 pe-1 flex items-center gap-1.5">
				{#if channel?.name}
	...
	{#if ['dm'].includes(channel?.type)}
		<div class="ms-0.5 me-1 hover-reveal self-center flex shrink-0 items-center dark:text-gray-300">
			<button
				type="button"
