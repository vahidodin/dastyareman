	...
	{#if loaded}
		...
		{#if collapsible}
			<Collapsible bind:open className="w-full" buttonClassName="w-full" onChange={setOpen}>
				<div class="flex items-center justify-between h-6 w-full ps-3.5 pe-1.5 shrink-0">
					<button
						type="button"
						class="group flex flex-1 h-full items-center gap-1 text-start text-xs text-gray-400 hover:text-gray-500 dark:text-gray-500 dark:hover:text-gray-400 transition-colors duration-100 {buttonClassName}"
						aria-expanded={open}
