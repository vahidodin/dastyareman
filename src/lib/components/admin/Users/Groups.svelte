...
{#if loaded}
	...
	<div>
			...
			<div class="flex h-8 flex-1 items-center w-full gap-2">
				...
				>
					...
					<svelte:fragment slot="item" let:item let:selected>
						{item.label}
						<div class="ms-auto {selected ? '' : 'invisible'}">
							<Check />
						</div>
					</svelte:fragment>
				</Select>
				...
				<button
					class="ms-1 shrink-0 rounded-lg bg-gray-50 px-2.5 py-1 text-xs text-gray-900 transition ring-1 ring-gray-200 hover:bg-gray-100 dark:bg-gray-850 dark:text-gray-100 dark:ring-gray-800 dark:hover:bg-gray-800"
					on:click={() => {
		...
		<button
			class="group flex cursor-pointer text-start w-full px-2.5 py-2"
		...
		>
			<div class="w-full">
				<div class="flex items-center gap-3">
					<div class="flex min-w-0 flex-1 flex-col gap-0.5 ps-1">
