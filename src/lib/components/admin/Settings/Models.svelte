		...
		<div class="flex h-full min-h-0 flex-col text-sm">
			<div class="mb-2 flex items-center justify-between">
				<h2 class="text-sm font-medium text-gray-900 dark:text-white">
					...
					<span class="ms-2 font-normal text-gray-500 dark:text-gray-500">
			...
			<div class="flex min-h-0 flex-1 flex-col space-y-1">
				...
				<div
					class="my-0.5 min-h-0 flex-1 space-y-px {filteredModels.length > 0
						? 'overflow-y-auto scrollbar-hover pe-1.5'
						: 'overflow-hidden'}"
					id="model-list"
				...
				>
					{#if filteredModels.length > 0}
						{#each filteredModels as model, modelIdx (`${model.id}-${modelIdx}`)}
							...
							>
								<div class="self-center pe-1 -ms-1 text-gray-400 dark:text-gray-600">
									<Tooltip
								...
								<button
									class="flex group/item gap-2.5 w-full min-w-0 flex-1 text-start cursor-pointer"
									type="button"
									on:click={() => {
								...
								>
									...
									<div
										class="flex min-w-0 flex-1 pe-1 self-center {(model?.is_active ?? true)
											? ''
											: 'text-gray-500'}"
									>
										<Tooltip
