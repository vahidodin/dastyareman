...
<div class="shrink-0">
	<div class="flex items-center justify-between gap-4 py-0.5">
		<button
			class="min-w-0 flex-1 text-start text-xs text-gray-600 transition hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100"
			type="button"
			on:click={() => {
				expanded = !expanded;
			}}
	...
	{#if expanded}
		...
		{:else}
			<div class="space-y-1 mt-0.5">
				<div>
					<button
						class="flex w-full items-center justify-between gap-4 py-0.5 text-start"
						type="button"
						on:click={() => {
					...
					{#if showCapabilities}
						<div
							class="max-h-[24rem] overflow-y-auto pb-2 pe-1 scrollbar-hover"
				...
				<div>
					...
					{#if showPromptSuggestions}
						<div
							class="max-h-[24rem] space-y-2 overflow-y-auto pb-2 pe-1 scrollbar-hover"
							on:click={updateDirty}
							on:change={updateDirty}
							on:input={updateDirty}
						>
