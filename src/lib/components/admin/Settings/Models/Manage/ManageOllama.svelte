<script lang="ts">
	...
	const inputClass =
		'h-7 w-full rounded-lg border border-gray-100/50 bg-gray-50/40 px-2.5 text-start text-xs text-gray-700 outline-hidden transition-colors focus:border-blue-400 disabled:opacity-50 dark:border-white/[0.04] dark:bg-white/[0.03] dark:text-gray-300 dark:focus:border-blue-500';
	const textareaClass =
		...
		<div>
			<div class="space-y-2">
						...
						{#each Object.keys($MODEL_DOWNLOAD_POOL) as model}
							{#if 'pullProgress' in $MODEL_DOWNLOAD_POOL[model]}
								<div class="flex flex-col">
									...
									<div class="">
										<div class="flex flex-row justify-between space-x-4 pe-2">
											<div class=" flex-1">
												<div
				...
				<div>
					...
					<div class="flex w-full">
						<div class="flex-1 me-2 flex flex-col gap-2">
							<input
								class={inputClass}
								placeholder={$i18n.t('Enter model tag (e.g. {{modelTag}})', {
				...
				{#if showExperimentalOllama}
					...
					>
						<div class=" mb-2 flex w-full justify-between">
							...
							>
								{#if modelUploadMode === 'file'}
									<span class="ms-2 self-center">{$i18n.t('File Mode')}</span>
								{:else}
									<span class="ms-2 self-center">{$i18n.t('URL Mode')}</span>
								{/if}
							</button>
						</div>
						...
						<div class="flex w-full mb-1.5">
							<div class="flex flex-col w-full">
								{#if modelUploadMode === 'file'}
									<div class="flex-1 {modelInputFile && modelInputFile.length > 0 ? 'me-2' : ''}">
										<input
								...
								{:else}
									<div class="flex-1 {modelFileUrl !== '' ? 'me-2' : ''}">
										<input
											class={inputClass}
											type="url"
											required
