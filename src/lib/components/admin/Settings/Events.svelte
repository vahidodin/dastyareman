								...
								{#if targetQuery.trim() && (targetGroupResults.length > 0 || targetUserResults.length > 0)}
									...
									>
										{#each targetGroupResults as group}
											<button
												type="button"
												class="w-full flex items-center justify-between gap-2 px-3 py-1.5 text-xs text-start hover:bg-gray-50/70 dark:hover:bg-gray-850/50"
							...
							<div class="max-h-36 overflow-y-auto pb-0.5">
								{#each filteredEvents as event}
									...
									>
										...
										<span class="min-w-0">
											<span class="font-mono break-all">{event}</span>
											<span class="ms-1 text-gray-500">{eventDetails[event]?.message}</span>
										</span>
									</label>
								{/each}
							</div>
