	...
	<div class="flex items-center gap-2 flex-wrap justify-end min-w-0">
		{#if groups.length > 0}
			<select
				...
				class="w-fit pe-8 rounded-sm px-2 text-xs bg-transparent outline-none text-end"
			>
...
{:else}
				...
				<table class="w-full text-sm text-start text-gray-500 dark:text-gray-400 table-auto">
					<thead class="text-xs text-gray-800 uppercase bg-transparent dark:text-gray-200">
						<tr class="border-b-[1.5px] border-gray-50 dark:border-gray-850/30">
							...
							<th
								scope="col"
								class="px-2.5 py-2 cursor-pointer select-none text-end"
							...
							<th
								scope="col"
								class="px-2.5 py-2 cursor-pointer select-none text-end w-16"
					...
					<tbody>
						{#each sortedModels as model, idx (model.model_id)}
							...
							>
								...
								<td class="px-3 py-1 text-end">{model.count.toLocaleString()}</td>
								<td class="px-3 py-1 text-end">{(model.unique_users ?? 0).toLocaleString()}</td>
								<td class="px-3 py-1 text-end">{(model.unique_chats ?? 0).toLocaleString()}</td>
								<td class="px-3 py-1 text-end"
									>{formatNumber(tokenStats[model.model_id]?.total_tokens ?? 0)}</td
								>
								<td class="px-3 py-1 text-end text-gray-400">
					...
					<tbody>
						{#each sortedUsers as user, idx (user.user_id)}
							<tr class="dark:border-gray-850 text-xs">
								...
								<td class="px-3 py-1 text-end">{user.count.toLocaleString()}</td>
								<td class="px-3 py-1 text-end">{formatNumber(user.total_tokens ?? 0)}</td>
							</tr>
						{/each}
	...
	<div class="text-gray-500 text-xs mt-1.5 text-end">
