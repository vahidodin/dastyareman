		...
		<div class="flex-1 flex flex-col min-h-0 px-3 pb-3">
			<div class="grid grid-cols-7">
				{#each DAY_NAMES as day}
					<div class="px-2 py-1.5 text-xs text-gray-400 dark:text-gray-500 text-start truncate">
						{$i18n.t(day)}
					</div>
				{/each}
			</div>
			...
			>
				{#each monthDays as day, i}
					...
					<button
						class="p-1 min-h-0 text-start overflow-hidden transition cursor-pointer flex flex-col
							{isCurrentMonth(day) ? '' : 'opacity-40'}
							hover:bg-gray-50/80 dark:hover:bg-gray-850/30
							{col > 0 ? 'border-s border-gray-100/20 dark:border-gray-850/20' : ''}
					...
					>
						...
						<div class="flex flex-col gap-0 flex-1 overflow-hidden">
							...
							{#if dayEvents.length > 3}
								...
								<div
									class="text-[0.625rem] text-gray-400 dark:text-gray-500 px-1 mt-auto hover:text-gray-700 dark:hover:text-gray-200 text-start w-full truncate z-10"
					...
					<div class="min-w-[43.75rem] flex flex-col flex-1">
							...
							{#each weekDays as day}
								<div
									class="text-center py-2.5 {day.getDay() > 0
										? 'border-s border-gray-100/20 dark:border-gray-850/20'
										: ''}"
								>
						...
						<div class="flex-1 overflow-y-auto">
							{#each hours as hour}
								...
								>
									<div
										class="text-[0.625rem] text-gray-400 dark:text-gray-500 text-end pe-2 select-none -mt-1.5 z-10"
									>
									...
									{#each weekDays as day}
										...
										<button
											class="px-0.5 py-0.5 {day.getDay() > 0
												? 'border-s border-gray-100/15 dark:border-gray-850/15'
		...
		<div class="flex-1 flex flex-col min-h-0 px-3 pb-3">
			...
			>
				{#each hours as hour}
					...
					>
						<div
							class="w-14 shrink-0 text-[0.625rem] text-gray-400 dark:text-gray-500 text-end pe-3 mt-1 select-none"
						>
							{formatHour(hour)}
						</div>
						<button
							class="flex-1 border-s border-gray-100/15 dark:border-gray-850/15 px-1.5 py-0.5
								hover:bg-gray-50/50 dark:hover:bg-gray-850/20 transition cursor-pointer flex flex-col text-start justify-start"
