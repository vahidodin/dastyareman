...
>
	<aside
		class={hideThumbs
			? 'hidden'
			: 'thumbnail-sidebar overflow-y-auto px-2 pt-3 pb-16 border-e border-gray-50 dark:border-gray-850/30 bg-transparent'}
	...
	>
		{#each slides as slide, index}
			<button
				...
				type="button"
				class="grid grid-cols-[20px_minmax(0,1fr)] items-start gap-2 w-full mb-3 p-0 text-start text-gray-900 dark:text-gray-100"
				on:click={() => selectSlide(index)}
				aria-label="{itemLabel} {index + 1}"
				aria-current={safeSlide === index ? 'true' : undefined}
			>
				<span
					class="pt-1.5 text-[0.6875rem] font-medium text-end {safeSlide === index
	...
	{#if slides.length > 0}
		<div
			class="absolute bottom-3 {hideThumbs
				? 'start-1/2'
				: 'start-[calc(144px+(100%-144px)/2)]'} -translate-x-1/2 z-10 flex items-center gap-0.5 rounded-lg bg-white/90 dark:bg-gray-850/90 backdrop-blur-sm shadow-lg border border-gray-200/60 dark:border-gray-700/60 px-1 py-0.5"
		>
			<button
				type="button"
