			...
			{#each filteredItems as item, i}
				...
				<Tooltip content={item?.id} placement="top-start">
					<button
						...
						class="flex items-center justify-between px-2.5 py-1.5 rounded-xl w-full text-start {i ===
						selectedIndex
					...
					>
						{#if item.type === 'channel'}
							<div class=" size-4 justify-center flex items-center me-0.5">
						...
						{:else if item.type === 'model'}
							<img
								...
								class="rounded-full size-5 items-center me-2"
								on:error={(e) => {
						...
						<div class="truncate flex-1 pe-2">
							{item.label}
						</div>
