	...
	<div class="py-3">
		<div class=" flex gap-3.5 w-full px-3 items-center">
			...
			<div class=" flex flex-col w-full flex-1">
				<div class="mb-0.5 font-normal line-clamp-1 pe-2">
					{user.name}
				</div>
			...
			<div class="mx-2 mt-2">
				<Tooltip content={user?.status_message}>
					...
					>
						...
						{/if}
						<div class=" self-center line-clamp-2 flex-1 text-start">
							{user?.status_message}
						</div>
					</div>
				</Tooltip>
		...
		{#if user?.bio}
			<div class="mx-3.5 mt-2">
				<Tooltip content={user?.bio}>
					<div class=" self-center line-clamp-3 flex-1 text-start text-xs">
						{user?.bio}
					</div>
				</Tooltip>
			</div>
		...
		{#if $_user?.id !== user.id}
			...
			<div class=" flex flex-col w-full px-2.5 items-center">
				<button
					class="w-full text-start px-3 py-1.5 rounded-xl border border-gray-100/50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-850 transition flex items-center gap-2 text-sm"
					type="button"
					on:click={() => {
