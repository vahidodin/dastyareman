	...
	>
		...
		<div
			...
			class="flex flex-col justify-between w-full max-w-full mx-auto group hover:bg-gray-300/5 dark:hover:bg-gray-700/5 relative {className
				...
				: `px-5 ${
						replyToMessage
							? 'border-s-4 border-blue-500 bg-blue-100/10 dark:bg-blue-100/5 ps-4'
							: ''
					} ${
						(message?.reply_to_message?.meta?.model_id ?? message?.reply_to_message?.user_id) ===
						$user?.id
							? 'border-s-4 border-orange-500 bg-orange-100/10 dark:bg-orange-100/5 ps-4'
							: ''
		...
		>
			{#if !edit && !disabled}
				<div class=" absolute {showButtons ? '' : 'hover-reveal'} end-1 -top-7 z-30">
					<div
			...
			{#if message?.is_pinned}
				<div class="flex {showUserProfile ? 'mb-0.5' : 'mt-0.5'}">
					<div class="ms-8.5 flex items-center gap-1 px-1 rounded-full text-xs">
			...
			{#if message?.reply_to_message?.user}
				<div class="relative text-xs mb-1">
					<div
						class="absolute h-3 w-7 start-[1.125rem] top-2 rounded-tl-lg border-t-[1.5px] border-s-[1.5px] border-gray-200 dark:border-gray-700 z-0"
					></div>

					<button
						class="ms-12 flex items-center space-x-2 relative z-0"
						on:click={() => {
					...
					>
						{#if message?.reply_to_message?.meta?.model_id}
							<img
								...
								class="size-4 ms-0.5 rounded-full object-cover"
								on:error={(e) => {
					...
					{#if showUserProfile}
						{#if message?.meta?.model_id}
							<img
								...
								class="size-8 translate-y-1 ms-0.5 object-cover rounded-full"
								on:error={(e) => {
									// LICENSE covers this Open WebUI fallback logo.
									// Do not alter, remove, obscure, or replace it except as LICENSE permits:
									// https://docs.openwebui.com/license.
				...
				<div class="flex-auto w-0 ps-2">
					{#if showUserProfile}
						<Name>
							...
							{#if message.created_at}
								<div
									class=" self-center text-xs text-gray-400 font-normal first-letter:capitalize ms-0.5 translate-y-[1px]"
								>
									<Tooltip content={dayjs(message.created_at / 1000000).format('LLLL')}>
							...
							{:else}
								<span class="markdown-prose">
									...
									/>
										...
										class="text-gray-500 text-[0.625rem] ps-1 self-center"
										>({$i18n.t('edited')})</span
									>{/if}
							{/if}
						</div>
