		...
		<div class="flex items-center w-full max-w-full">
			{#if $mobile}
				<div
					class="{$showSidebar
						? 'md:hidden'
						: ''} me-1.5 mt-0.5 self-start flex flex-none items-center text-gray-600 dark:text-gray-400"
				>
					<Tooltip
					...
					<div class="flex items-center gap-0.5 shrink-0">
						{#if channel?.type === 'dm'}
							{#if channel?.users}
								...
								<div class="flex me-1.5 relative">
							...
							{:else}
								<Users className="size-4 ms-1 me-0.5" strokeWidth="2" />
							{/if}
						{:else}
						...
						<div class=" text-start self-center overflow-hidden w-full line-clamp-1 flex-1">
							{#if channel?.name}
								{channel.name}
							{:else}
								{channel?.users
