...
<div
	class="group flex h-[1.6875rem] w-full cursor-pointer select-none items-center rounded-xl px-2 text-start text-[0.8125rem] font-normal text-gray-700 outline-hidden transition-colors duration-75 hover:bg-gray-50/40 dark:text-gray-100 dark:hover:bg-gray-800/40 [&_button:hover]:bg-transparent! dark:[&_button:hover]:bg-transparent! {selected
		? 'bg-gray-50/70 dark:bg-gray-800/60'
		: ''}"
>
	<button
		type="button"
		class="flex h-full min-w-0 flex-1 items-center gap-2 overflow-hidden p-0 text-start outline-hidden"
	...
	<div
		class="{showMenu ? 'visible' : 'hover-reveal'} ms-auto flex shrink-0 items-center gap-1.5 ps-2"
	>
		<Dropdown
			bind:show={showMenu}
			align="end"
