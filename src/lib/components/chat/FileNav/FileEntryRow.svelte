...
<li class="group" data-file-row>
	...
	>
		{#if entry.type === 'directory'}
			<button
				type="button"
				class="me-1.5 flex w-5 shrink-0 items-center self-stretch justify-center text-gray-400 dark:text-gray-600 hover:text-gray-600 dark:hover:text-gray-400"
		...
		{:else}
			<span class="me-1.5 w-5 shrink-0 self-stretch" style="margin-left: {rowIndent};"></span>
		{/if}
		...
		{#if selectionMode || selected}
			<button
				...
				class="me-2 flex size-3.5 shrink-0 items-center justify-center rounded border transition-colors {selected
		...
		<button
			type="button"
			class="flex min-w-0 flex-1 items-center gap-2 py-1.5 pe-2 text-start"
			draggable={canMutate}
			on:dragstart={(e) => {
				if (!canMutate) {
					e.preventDefault();
		...
		<Dropdown bind:show={menuOpen} align="end" sideOffset={4}>
			<button
				class="shrink-0 flex h-5 w-5 items-center justify-center me-1 rounded transition
