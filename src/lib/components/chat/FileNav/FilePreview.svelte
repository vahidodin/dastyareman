...
>
	...
	{:else if fileContent !== null}
		...
		{:else if isHtml && !showRaw && serveUrl}
			{#if overlay}
				<div class="absolute top-0 start-0 end-0 bottom-0 z-10"></div>
			{/if}
			<iframe
				src={serveUrl}
	...
	{#if !fileLoading && fileImageUrl !== null}
		<div
			class="absolute bottom-3 start-1/2 z-10 flex -translate-x-1/2 items-center gap-0.5 rounded-lg border border-gray-200/60 bg-white/90 px-1 py-0.5 shadow-lg backdrop-blur-sm dark:border-gray-700/60 dark:bg-gray-850/90"
		>
