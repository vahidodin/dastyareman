					...
					{:else}
						{#if filenameMatches.length > 0}
							...
							{#each filenameMatches as match (match.path)}
								<button
									class="flex h-7 w-full items-center gap-2 rounded-lg px-2 text-start transition-colors duration-75 hover:bg-gray-50 dark:hover:bg-white/4"
								...
								>
									...
									<span class="min-w-0 flex-1 truncate text-xs text-gray-800 dark:text-gray-200">
										...
										{#if relativeParentPath(match.relative_path)}
											<span class="ms-1.5 text-[0.6875rem] text-gray-400 dark:text-gray-600">
								...
								{#if match.content_matches.length > 0}
									...
									<button
										class="flex h-6 w-full items-center gap-2 rounded-lg ps-8 pe-2 text-start transition-colors duration-75 hover:bg-gray-50 dark:hover:bg-white/4"
										on:click={() => openContentMatch(match, preview)}
									>
										<span
											class="w-6 shrink-0 text-end font-mono text-[0.625rem] text-gray-400 dark:text-gray-600"
											>{preview.line}</span
										>
										<span
