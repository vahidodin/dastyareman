		...
		<div class="min-h-0 flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin">
			...
			{:else}
				{#each items as item, idx (item.id)}
					<div
						class=" h-[1.6875rem] px-2 rounded-xl w-full text-start flex justify-between items-center text-[0.8125rem] font-normal hover:bg-gray-50/40 hover:text-gray-900 dark:hover:bg-gray-800/40 dark:hover:text-gray-100 {idx ===
						selectedIdx
					...
					>
						...
						>
							<div class="w-full text-start text-black dark:text-gray-100 flex items-center gap-1">
						...
						<Tooltip content={$i18n.t('Show Files')} placement="top">
							<button
								type="button"
								class=" ms-2 opacity-50 hover:opacity-100 transition"
								on:click={() => {
					...
					{#if selectedItem && selectedItem.id === item.id}
						<div class="ps-3 mb-1 flex flex-col gap-0.5">
							...
							{:else}
								{#each selectedFileItems as file, fileIdx (file.id)}
									<button
										class=" h-[1.6875rem] px-2 rounded-xl w-full text-start flex justify-between items-center text-[0.8125rem] font-normal hover:bg-gray-50/40 hover:text-gray-900 dark:hover:bg-gray-800/40 dark:hover:text-gray-100"
										type="button"
										on:click={() => {
											console.log(file);
											onSelect({
