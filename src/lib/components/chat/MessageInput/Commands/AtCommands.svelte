...
{#if knowledgeResults.length > 0 || query.startsWith('http')}
	{#each knowledgeResults as item, idx}
		...
		<button
			class="flex h-[1.6875rem] w-full max-w-full items-center justify-between overflow-hidden rounded-xl px-2 text-start text-[0.8125rem] hover:bg-gray-50/40 dark:hover:bg-gray-800/40 {itemIdx ===
			selectedIdx
			...
			>
				<div class="flex min-w-0 items-center text-black dark:text-gray-100">
					<img
						...
						class="me-2 size-4.5 rounded-full object-cover"
						on:error={(e) => {
