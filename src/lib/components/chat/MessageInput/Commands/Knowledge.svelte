...
{#if filteredItems.length > 0 || query.startsWith('http')}
	{#each filteredItems as item, idx}
		...
		{#if !['youtube', 'web'].includes(item.type)}
			<button
				class="flex h-[1.6875rem] w-full items-center justify-between rounded-xl px-2 text-start text-[0.8125rem] hover:bg-gray-50/40 dark:hover:bg-gray-800/40 {idx ===
				selectedIdx
	...
	{#if isYoutubeUrl(query)}
		<button
			class="flex h-[1.6875rem] w-full items-center rounded-xl bg-gray-50/40 px-2 text-start text-[0.8125rem] dark:bg-gray-800/40 dark:text-gray-100 selected-command-option-button"
			type="button"
