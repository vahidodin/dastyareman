	...
	>
		{#if !expanded}
			<button
				...
				class="flex h-full min-w-0 flex-1 items-center gap-1.5 px-2 text-start hover:text-gray-900 dark:hover:text-gray-100"
			>
				...
				{#if !shellDismissed}
					...
					>
						<button
							...
							class="tab-button flex h-full items-center gap-1.5 ps-2 pe-1"
							title={$i18n.t('Shell')}
						>
						...
						{#if shellOpened}<button
								on:click={closeShell}
								class="tab-close me-1 flex h-5 w-4 items-center justify-center"
