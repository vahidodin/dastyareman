<script lang="ts">
	...
	const tabButtonClass = (active: boolean) =>
		`flex items-center gap-1.5 h-7 px-2 md:w-full shrink-0 rounded-lg text-xs text-start transition-colors duration-75 ${
			active
...
>
	<nav
		...
		class="shrink-0 min-w-0 md:min-h-0 flex flex-col border-b md:border-b-0 md:border-e border-gray-100/30 dark:border-white/[0.02] md:w-[15rem]"
	>
		<button
		...
		>
			<div class="self-center rounded-s-xl bg-transparent">
		...
		<div
			class="tabs scrollbar-none max-h-32 md:max-h-none flex min-w-0 flex-1 min-h-0 overflow-x-auto md:overflow-x-hidden md:overflow-y-auto md:flex-col p-1 ps-0 md:ps-1 gap-px"
		>
			<span
