...
>
	{#if !loading}
		<div in:fade={{ duration: 50 }} class="w-full h-full flex flex-col">
			{#if backgroundImage}
				<div
					class="pointer-events-none absolute top-0 start-0 w-full h-full bg-cover bg-center bg-no-repeat"
					style="background-image: url({backgroundImage})"
				/>
				<div
					class="pointer-events-none absolute top-0 start-0 w-full h-full bg-linear-to-t from-white to-white/85 dark:from-gray-900 dark:to-gray-900/90 z-0"
				></div>
			{/if}
						...
						{#if ($settings?.landingPageMode === 'chat' && !$selectedFolder) || createMessagesList(history, history.currentId).length > 0}
							...
							{:else}
								...
								>
									...
									<div
										class="absolute bottom-1 text-xs text-gray-500 text-center line-clamp-1 end-0 start-0"
									>
										...
										<div class="w-full">
											...
											<div class="flex flex-col">
												{#each suggestedPrompts as suggestion}
													<button
														type="button"
														class="flex min-h-8 w-full items-center justify-between py-1 text-start text-[0.8125rem] leading-5 text-gray-500 transition hover:text-gray-700 dark:text-gray-500 dark:hover:text-gray-300"
														on:click={async () => {
															await tick();
