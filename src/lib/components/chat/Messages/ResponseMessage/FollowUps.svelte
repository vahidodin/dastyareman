...
<div class="mt-4">
	...
	<div class="flex flex-col text-start gap-1 mt-1.5">
		{#each followUps as followUp, idx (idx)}
			<Tooltip content={followUp} placement="top-start" className="line-clamp-1">
				<button
					class=" py-1 bg-transparent text-start text-sm flex items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition cursor-pointer w-full"
