...
>
	{#if !($settings?.chatBubble ?? true) && !(message?.meta?.internal === true && message?.meta?.type === 'subagent') && !(message?.meta?.internal === true && message?.meta?.type === 'timer')}
		<div class={`shrink-0 ltr:me-2 rtl:ms-2 hidden @lg:flex mt-0.5`}>
		...
		<div class="chat-{message.role} w-full min-w-full">
			...
			{:else if message?.meta?.internal === true && message?.meta?.type === 'timer'}
				<div class="w-full min-w-0">
					<button
						type="button"
						class="flex w-full min-w-0 items-center gap-2 text-start text-gray-500 transition-colors hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
					...
					{#if timerExpanded}
						<div
							class="mt-2 ms-3 whitespace-pre-wrap break-words border-s border-gray-100 ps-3 text-[0.78125rem] leading-relaxed text-gray-600 dark:border-white/10 dark:text-gray-400"
							dir={$settings?.chatDirection ?? 'auto'}
						>
							{message.content}
						</div>
			...
			{#if edit !== true && !(message?.meta?.internal === true && message?.meta?.type === 'subagent') && !(message?.meta?.internal === true && message?.meta?.type === 'timer')}
				...
				>
					{#if message.timestamp}
						<Tooltip
							className="flex self-center {($settings?.chatBubble ?? true) ? 'me-1' : 'order-last'}"
						...
						>
							<time
								...
								class="{compactPreview ? '' : 'hover-reveal'} {($settings?.chatBubble ?? true)
									? 'me-1'
									: 'ms-1 shrink-0 whitespace-nowrap'} text-[0.6875rem] tabular-nums text-gray-400 dark:text-gray-600 select-none"
							>
