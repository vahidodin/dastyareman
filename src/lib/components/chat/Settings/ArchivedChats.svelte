...
<div id="tab-archived-chats" class="flex flex-col h-full text-sm">
			...
			<div slot="content">
				<DropdownMenu className="w-[10.625rem] shadow-sm">
					...
					>
						...
						{/if}
						<div class="min-w-0 flex-1 truncate text-start">
	...
	<div class="mt-3 flex-1 min-h-0 overflow-y-auto scrollbar-hover pe-1.5">
		{#if chatList === null}
		...
		{:else}
			<div class="flex items-center px-1 text-xs text-gray-400 dark:text-gray-600">
				<button
					class="flex flex-1 items-center gap-1 py-0.5 text-start"
					type="button"
				...
				<button
					class="hidden w-24 items-center justify-end gap-1 py-0.5 text-end sm:flex"
					type="button"
			...
			<div>
				{#each chatList as chat (chat.id)}
					<div class="flex w-full justify-between gap-2 px-1 py-1 text-xs">
						<a
							class="me-auto min-w-0 max-w-xl flex-1 self-center truncate text-gray-600 hover:text-black dark:text-gray-400 dark:hover:text-white"
							href={`/c/${chat.id}`}
							title={chat?.title}
							on:click={(event) => {
