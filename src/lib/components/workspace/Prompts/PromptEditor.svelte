...
{#if edit}
	...
	<div class="flex h-full max-h-[100dvh] w-full flex-col">
		...
		<div class="flex shrink-0 items-start justify-between gap-3 pb-1">
			...
			<div class="flex shrink-0 items-center gap-1.5 pe-0.5">
				{#if !disabled}
					<button
		...
		<div class="flex flex-1 flex-col gap-3 overflow-hidden pb-4 md:flex-row">
			...
			<div class="flex-1 flex flex-col min-h-0 overflow-hidden">
				...
				<div class="relative flex-1 min-h-0">
					...
					>
						<pre
							class="whitespace-pre-wrap pe-8 font-mono text-[0.6875rem] leading-relaxed">{selectedHistoryEntry
	...
	<div class="w-full max-h-full {modal ? 'h-full flex flex-col' : ''}">
		{#if modal}
			<div class="flex justify-between items-center dark:text-gray-100 px-5 pt-4 pb-2">
				...
				<button
					class="self-center shrink-0 ms-2"
					aria-label={$i18n.t('Close')}
					type="button"
					on:click={() => {
						onCancel();
		...
		{#if history.length > 0}
			<div class="space-y-0 flex-1 overflow-y-auto" on:scroll={handleHistoryScroll}>
				{#each history as entry, index}
					<button
						class="group relative w-full px-1.5 py-1.5 ps-3 text-start transition {selectedHistoryEntry?.id ===
						entry.id
					...
					>
						<span
							class="absolute start-0 top-1.5 h-[calc(100%-0.75rem)] w-px rounded-full transition {selectedHistoryEntry?.id ===
							entry.id
						...
						<div class="flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500">
							{#if entry.user}
								<img
									...
									alt={entry.user.name}
									class="size-3 rounded-full me-0.5"
