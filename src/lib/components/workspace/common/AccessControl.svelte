...
<div class="rounded-lg flex flex-col gap-1">
	<div class="py-1.5">
		<div class="flex gap-2 items-center">
			...
			<div>
				...
				>
					<select
						id="models"
						class="outline-none bg-transparent text-sm font-normal block w-fit pe-8 max-w-full placeholder-gray-400"
		...
		{#if hasPublicReadGrant(accessGrants ?? []) && !hasAnyoneReadGrant(accessGrants ?? []) && accessRoles.includes('write')}
			<div class="flex w-full justify-between mt-1.5 ms-0.5">
					...
					<div class="flex justify-end items-center gap-1.5 shrink-0">
						{#if accessRoles.includes('write')}
							<select
								...
								class="bg-transparent text-sm outline-none pe-5"
