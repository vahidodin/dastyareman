...
async def lifespan(app: FastAPI):
    ...
    from open_webui.utils.branding import apply_runtime_branding

    await apply_runtime_branding(app)
...
async def get_app_config(request: Request):
    ...
    return {
        **({'onboarding': True} if onboarding else {}),
        ...
        'tagline': config.get('ui.branding.tagline') or '',
        'version': VERSION,
...
async def get_manifest_json():
    ...
    if external_pwa_manifest_url:
        ...
        return {
            ...
            'description': (await Config.get('ui.branding.tagline'))
            or f'{app.state.WEBUI_NAME}',
            'start_url': '/',
            'display': 'standalone',
