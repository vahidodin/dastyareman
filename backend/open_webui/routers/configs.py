...
@router.get('/branding')
async def get_branding_config(user=Depends(get_admin_user)):
    values = await Config.get_many('ui.branding.name', 'ui.branding.tagline')
    return {
        'name': values.get('ui.branding.name') or '',
        'tagline': values.get('ui.branding.tagline') or '',
    }


@router.post('/branding')
async def set_branding_config(request: Request, form_data: BrandingForm, user=Depends(get_admin_user)):
    name = form_data.name.strip()
    if not name:
        raise HTTPException(status_code=400, detail='Brand name is required')
    await Config.upsert(
        {
            'ui.branding.name': name,
            'ui.branding.tagline': form_data.tagline.strip(),
        }
    )
    request.app.state.WEBUI_NAME = name
    return {'name': name, 'tagline': form_data.tagline.strip()}


@router.post('/branding/assets/{slot}')
async def upload_branding_asset(
    ...
    from open_webui.utils.branding import ASSET_TARGETS, save_branding_asset

    if slot not in ASSET_TARGETS:
        raise HTTPException(status_code=400, detail='Unknown branding asset')
    data = await file.read()
    if len(data) > 8 * 1024 * 1024:
        raise HTTPException(status_code=400, detail='Branding file is larger than 8 MB')
    try:
        save_branding_asset(slot, data)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return {'slot': slot, 'size': len(data)}
