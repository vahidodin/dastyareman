...
# CUSTOM_NAME (removed)
####################################
# The upstream CUSTOM_NAME path calls api.openwebui.com to fetch a hosted logo.
# Branding is stored locally instead. See open_webui.utils.branding.
...
PGVECTOR_INITIALIZE_MAX_VECTOR_LENGTH = int(os.getenv('PGVECTOR_INITIALIZE_MAX_VECTOR_LENGTH', '1024'))
...
RAG_TOP_K_RERANKER = int(os.getenv('RAG_TOP_K_RERANKER', '5'))
...
ENABLE_RAG_HYBRID_SEARCH = os.getenv('ENABLE_RAG_HYBRID_SEARCH', 'true').lower() == 'true'
...
RAG_EMBEDDING_BATCH_SIZE = int(
    os.getenv('RAG_EMBEDDING_BATCH_SIZE') or os.getenv('RAG_EMBEDDING_OPENAI_BATCH_SIZE', '16')
)
...
# multilingual-e5 requires these prefixes; other models ignore them when unset via env "".
RAG_EMBEDDING_QUERY_PREFIX = os.getenv('RAG_EMBEDDING_QUERY_PREFIX', 'query: ')

RAG_EMBEDDING_CONTENT_PREFIX = os.getenv('RAG_EMBEDDING_CONTENT_PREFIX', 'passage: ')
...
Answer the user using only the context below. Cite a source with [id] only when that source tag has an id attribute (for example <source id="1">).

### Guidelines:
- Reply in the same language as the user's question. For a Persian question, answer in Persian.
- If the context contains the answer, state it directly and cite the source ids that support it.
- If the context only partially answers the question, give that partial answer, cite it, and say which part is missing.
- If the answer is not in the context, say clearly that it was not found in the provided documents. Do not invent document content and do not fill the gap from general knowledge.
- If the context is unreadable, say so.
- Do not use XML tags in the answer.
- Keep citations short, like [1] or [2].

### Output:
A direct answer, with [id] citations only where a source id is present.
...
DEFAULT_LOCALE = os.getenv('DEFAULT_LOCALE', 'fa-IR')
...
DEFAULT_PROMPT_SUGGESTIONS_I18N = {
    'fa-IR': {
        'suggestion_prompts': [
            {
                'title': ['خلاصه‌اش کن', 'این سند را در چند نکته'],
                'content': 'این سند را بخوان و مهم‌ترین نکته‌ها را به فارسی و به‌صورت فهرست خلاصه کن.',
            },
            {
                'title': ['از روی مدرک جواب بده', 'و منبع را مشخص کن'],
                'content': 'بر اساس مدارک بارگذاری‌شده به این پرسش جواب بده و شماره منبع را ذکر کن.',
            },
            {
                'title': ['تفاوت‌ها را پیدا کن', 'بین دو بخش از سند'],
                'content': 'تفاوت‌های مهم بین بخش‌های مرتبط این سند را روشن و کوتاه توضیح بده.',
            },
            {
                'title': ['یک چک‌لیست بساز', 'از الزام‌های این متن'],
                'content': 'از این متن یک چک‌لیست عملی از الزام‌ها و اقدام‌های لازم استخراج کن.',
            },
        ]
    }
}
...
DEFAULT_CONFIG = {
    ...
    'ui.branding.name': WEBUI_NAME,
    'ui.branding.tagline': os.getenv('WEBUI_TAGLINE', ''),
